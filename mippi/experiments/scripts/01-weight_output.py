...
'''
att51_ori = [np.zeros((data[0].shape[0], 4, 51, 51))] * 3
att51_mut = [np.zeros((data[0].shape[0], 4, 51, 51))] * 3
att51_mix = [np.zeros((data[0].shape[0], 4, 51, 51))] * 3
...
data_mix = [mut0_c, mut1_c, par0_c, pssm_win_mut0, pssm_win_mut1, pssm_par0]
for i in range(3): # 3 transformer layers
    for j in range(5): # 5-fold
        ...
        att51_mix[i] = att51_model.predict(data_mix)[1] + att51_mix[i]

for i in range(3):
    att51_ori_sum = att51_ori[i].sum(axis=-2)
    print(att51_ori_sum.shape) #(16505, 4, 51)
    np.save('att_weight/att51_ori/att51_ori_weight_' + str(i) + '.npy', att51_ori_sum)
    att51_mut_sum = att51_mut[i].sum(axis=-2)
    print(att51_mut_sum.shape) #(16505, 4, 51)
    np.save('att_weight/att51_mut/att51_mut_weight_' + str(i) + '.npy', att51_mut_sum)
    att51_mix_sum = att51_mix[i].sum(axis=-2)
    print(att51_mix_sum.shape) #(16505, 4, 51)
    np.save('att_weight/att51_mix/att51_mix_weight_' + str(i) + '.npy', att51_mix_sum)    
'''
...
while(seq2 <= data[0].shape[0]):
    ...
    for i in range(3):
        for j in range(5):
            model_path = './via_att0/bestAcc.h5' + str(j)
            model.load_weights(model_path)
            att51_model = keras.Model(model.input, [model.output, model.get_layer('transformer_block' + layer_name[i]).output])
            att51_ori[i] = att51_model.predict(data_ori)[1][1] + att51_ori[i]
            att51_mut[i] = att51_model.predict(data_mut)[1][1] + att51_mut[i]
            
    for i in range(3):
        partner_wei_sum = att51_ori[i].sum(axis=-2)
        print(partner_wei_sum.shape) #(step, 4, 1024)
        for t in range(step):
            print(partner_wei_sum[t].shape)
            np.save('att_weight/att1024_partner/att1024_partner_' + str(seq1 + t) + '_weight_' + str(i) + '.npy', partner_wei_sum[t])    
