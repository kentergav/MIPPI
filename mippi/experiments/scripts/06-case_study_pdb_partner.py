...
print(att51_ori[0].shape) # (193, 4, 1024, 1024)
att51_ori_sum = att51_ori[2][1].sum(axis=-2) 
print(att51_ori_sum.shape)

for stage in range(3):
    dataset = []
    for i in range(att51_ori[0].shape[0]):
        dataset.append([df_s['parAC'][i], df_s['par_seq_len'][i], att51_ori[stage][i].sum(axis=-2)])
    dataset_np = np.array(dataset)
    print(dataset_np.shape)
    #print(dataset_np)
    np.save('case_study/case_study_stage_' + str(stage) + '_dataset.npy', dataset_np)
